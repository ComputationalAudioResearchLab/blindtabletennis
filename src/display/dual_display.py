...
class Canvas(vispy.app.Canvas):
    ...
    def on_draw(self, event):
        ...
        status, packet = self.devices[self.index].__next__()
        if status.ring is None:
            self.backlogs[:] += 1
            self.backlogs[self.index] = 0
        else:
            assert packet is not None
            backlog = status.ring.backlog()
            self.backlogs[:] += 1
            self.backlogs[self.index] = backlog
            if "dvs_events" in packet:
                assert status.ring is not None and status.ring.current_t is not None
                self.current_ts[self.index] = np.float32(status.ring.current_t)
                self.program["u_t"] = self.current_ts.max()
                self.ts_and_ons[
                    packet["dvs_events"]["x"],
                    packet["dvs_events"]["y"] + self.sensor_height * self.index,
